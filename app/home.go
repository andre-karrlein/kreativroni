...
func (home *home) Render() app.UI {
	return app.Div().Class("body w-full h-full bg-gradient-to-r from-green-200 to-green-500 p-0 pt-5 md:p-8 md:py-10").Body(
		&navbar{},
		app.Div().Class("pt-0 md:pt-20").Body(
			app.Figure().Class("bg-white bg-opacity-40 backdrop-filter backdrop-blur-lg rounded-xl p-4 md:flex md:p-30").Body(
				...
				app.Img().Class("w-32 h-32 md:w-48 md:h-auto md:rounded-none rounded-full mx-auto").Src("https://storage.googleapis.com/kreativroni-web/Haaand.PNG"),
			),
		),
	)
}
